---
title: "Chainlink VRF in Solidity: Secure Random Numbers for Blockchain Games"
description: "Learn to implement Chainlink VRF v2 in Solidity for provably fair randomness in blockchain games. Complete guide with code examples, security patterns, and best practices for Web3 developers."
image: "/assets/guides/chainlink-vrf-solidity-games.jpg"
---

## TL;DR: Chainlink VRF for Secure Gaming
...
## 5. Building Your Dice Game Contract

### Complete DiceGame Implementation
...
/**
 * @title Secure Dice Game using Chainlink VRF v2
 * @notice Demonstrates provably fair randomness in gaming
 */
contract DiceGame is VRFConsumerBaseV2 {
    // Events for transparency
    event DiceRolled(uint256 indexed requestId, address indexed player);
    event DiceResult(uint256 indexed requestId, address indexed player, uint256 result);
    ...
    mapping(address => uint256) public s_rollCount;
    ...
    /**
     * @notice Request a random dice roll
    ...
    function rollDice() external returns (uint256 requestId) {
        ...
        // Store the player who made the request
        s_rollers[requestId] = msg.sender;
        s_rollCount[msg.sender]++;
    ...
    function fulfillRandomWords(
        ...
        // Store result for the player
        s_results[player] = diceResult;
        
        // Clean up mapping to save gas
        delete s_rollers[requestId];

        emit DiceResult(requestId, player, diceResult);
    }

    // View functions for checking game state
    function getLastResult(address player) external view returns (uint256) {
        return s_results[player];
    }

    function getRollCount(address player) external view returns (uint256) {
        return s_rollCount[player];
    }
}
```
...
async function main() {
    ...
    const DiceGame = await ethers.getContractFactory("DiceGame");
    const diceGame = DiceGame.attach(contractAddress);

    console.log("Rolling dice...");
    const tx = await diceGame.rollDice();
    const receipt = await tx.wait();
    
    // Extract request ID from events
    const event = receipt.events?.find(e => e.event === "DiceRolled");
    ...
    while (result === 0) {
        ...
        result = await diceGame.getLastResult(signer.address);
        if (result > 0) {
...
### 8.0 Request Batching for High-Throughput Games

For games with many simultaneous players, batching requests improves efficiency:

```solidity
contract BatchDiceGame is VRFConsumerBaseV2 {
    struct BatchRequest {
        address[] players;
        uint256 timestamp;
        bool fulfilled;
    ...
    function joinBatch() external {
        require(playerBatches[msg.sender] == 0, "Already in batch");
        
        if (currentBatchId == 0 || 
            batchRequests[currentBatchId].players.length >= MAX_BATCH_SIZE) {
            currentBatchId = requestNewBatch();
        }
        
        batchRequests[currentBatchId].players.push(msg.sender);
        playerBatches[msg.sender] = currentBatchId;
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) 
        ...
        // Distribute random results to all players in batch
        uint256 seed = randomWords[0];
        for (uint256 i = 0; i < batch.players.length; i++) {
            uint256 playerSeed = uint256(keccak256(abi.encode(seed, i)));
            uint256 roll = (playerSeed % 6) + 1;
            s_results[batch.players[i]] = roll;
        }
...
Production games need circuit breakers for critical situations:

```solidity
import "@openzeppelin/contracts/security/Pausable.sol";
import "@openzeppelin/contracts/access/AccessControl.sol";

contract SecureDiceGame is VRFConsumerBaseV2, Pausable, AccessControl {
    ...
    function rollDice() external emergencyStop returns (uint256) {
        // Normal dice roll logic
    }
    ...
    function pauseGame(string calldata reason) external onlyRole(PAUSER_ROLE) {
        _pause();
        emit GamePaused(msg.sender, reason, block.timestamp);
    }
    
    function unpauseGame() external onlyRole(PAUSER_ROLE) {
        _unpause();
        emit GameUnpaused(msg.sender, block.timestamp);
    }
}
```
...
**Ready to implement VRF in your game? [Try the Dice Game Challenge!](/challenge/dice-game)**

**Want to learn more about game security? [Check out the Blockchain Games Security Guide!](/guides/blockchain-games-vulnerabilities)**

**Building an NFT game? [Explore NFT Use Cases Beyond Art!](/guides/nft-use-cases)**