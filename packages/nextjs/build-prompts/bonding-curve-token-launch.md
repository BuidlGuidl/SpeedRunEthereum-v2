...
imageUrl: "/assets/build-prompts/token-launcher.png"
featured: true
---
...
- `UI_THEME`: Vibrant trading terminal with live price charts and activity feed. Update the SE-2 theme to match.
...
- **Required Views:** Implement a React component for each feature.
...
- **Agent Autonomy:** Build the `UI_THEME`. The price chart is the hero element: plot the bonding curve visually and show a marker for the current position on it. When the user types a buy/sell amount, dynamically highlight the area on the curve that their trade would traverse. The activity feed should auto-update and make the page feel alive. Use the Scaffold-ETH 2 `Address` component to display addresses (token creator, traders) — do not render raw hex strings.
